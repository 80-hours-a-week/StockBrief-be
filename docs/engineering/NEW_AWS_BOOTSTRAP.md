...
2. BE repo 루트에서 bootstrap script를 실행한다. `--environment`에는 팀원
   ...
   scripts/bootstrap_github_oidc.sh \
     ...
     --vpc-id vpc-xxxxxxxx \
     --db-subnet-ids subnet-private-a,subnet-private-b \
     --lambda-subnet-ids subnet-private-a,subnet-private-b \
     --vpc-endpoint-route-table-ids rtb-private \
     --alarm-emails-json '["REPLACE_WITH_ALERT_EMAIL"]'
   ```

   네트워크 ID가 아직 확정되지 않은 초기 점검이면 `--vpc-id`,
   `--db-subnet-ids`, `--lambda-subnet-ids`,
   `--vpc-endpoint-route-table-ids`를 빼고 실행한다. 이 경우 스크립트는
   skeleton `TFVARS_JSON`을 GitHub Environment에 쓰며, RDS/VPC 연결 profile은
   네트워크 검토 후 다시 생성한다.

3. 출력된 state bucket, lock table, deploy role ARN을 내부 핸드오프에
   기록한다. credential이나 secret value는 기록하지 않는다. 이 명령에
   `--write-deploy-profile-vars`를 포함했다면 GitHub Environment에는
   `AWS_<TARGET_ENV>_DEPLOY_ROLE_ARN`, `OPERATIONAL_ALARM_EMAILS_JSON`,
   `TF_BACKEND_CONFIG_HCL`, `TFVARS_JSON`이 함께 등록된다.

4. `--write-deploy-profile-vars`를 사용하지 않았거나 profile 파일을 직접
   검토하려면 backend profile 파일을 만든다.
...
5. `--write-deploy-profile-vars`를 사용하지 않았거나 profile 파일을 직접
   검토하려면 tfvars profile 파일을 만든다.
...
7. bootstrap에서 `--write-deploy-profile-vars`를 사용했다면 GitHub
   Environment variable 등록은 이미 끝났다. 사용하지 않은 경우에는 직접
   등록한다. 실제 account/resource ID가 들어간 profile 파일을 repo에
   커밋하지 않는 기본 운영에서는 Actions가 이 값을 읽어서 runner 안에 임시
   profile 파일을 만든다.

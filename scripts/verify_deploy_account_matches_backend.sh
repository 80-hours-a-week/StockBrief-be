...
if [ ! -f "$backend_file" ]; then
  echo "::error::Terraform backend file not found: ${backend_file}"
  exit 1
fi

backend_bucket="$(awk -F'"' '/bucket[[:space:]]*=/{print $2; exit}' "$backend_file")"
...
echo "Verified deploy account ${actual_account} matches deploy role and Terraform backend."
